    ...
    public static void main(String[] args) throws IOException {
        ...
        for(int i = 1; i < N+1; i++){
            for(int stamina=MAX_STAMINA; stamina > 0; stamina--) {   //뒤에서 부터 특정 체력에 대한 기쁨을 갱신하면서 반복
                int total = stamina-exhaustion[i];  //현재 체력과 체력소모를 더해서 현재 상태에서 가능한 상황을 고려
                if (total > 0) {
                    dp[stamina] = Math.max(dp[stamina], dp[total] + happy[i]);    //"이미 갱신된 기쁨"과 "현재 체력에 대한 기쁨"을 비교하기
        ...
        System.out.println(dp[MAX_STAMINA]);
    }
}